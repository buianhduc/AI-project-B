...
class ABAgent:
    ...
    def get_score(self, board: Board, player_color: PlayerColor) -> int:
        count = 0
        target_r = 7 if player_color == PlayerColor.RED else 0
        from_r = 0 if player_color == PlayerColor.RED else 7

        forward_dirs = {
            PlayerColor.RED: [Direction.Down, Direction.DownLeft, Direction.DownRight],
            PlayerColor.BLUE: [Direction.Up, Direction.UpLeft, Direction.UpRight],
        }
        position_of_frogs: set[Coord] = set(
            coord for coord, cell in board._state.items()
            if cell.state == player_color 
        )
        # moving all frogs together (avoid leaving frogs behind)
        rows = [frog.r for frog in position_of_frogs]
        row_range = max(rows) - min(rows)
        count -= row_range * 5

        for frog in position_of_frogs:
            r_dis = abs(frog.r - target_r) 
            count -= r_dis * 10


            move_options = self.get_neighbors_and_distance(
                player_color, board, frog, current_direction=[]
            )
            
            # penalize blocking when frog has limited move forward options
            for dir in forward_dirs[player_color]:
                try:
                    in_front = frog + dir
                    if board[in_front].state == player_color:
                        forward_moves = [move for move in move_options
                                if isinstance(move[0]._directions, list)
                                and move[0]._directions[0] in forward_dirs[player_color]]
