...
ILLEGAL_RED_DIRECTIONS = {Direction.Up, Direction.UpRight, Direction.UpLeft}

ILLEGAL_BLUE_DIRECTIONS = {Direction.Down, Direction.DownRight, Direction.DownLeft}
...
BoardState = dict[Coord, CellState]
...
class Board:
    ...
    def _assert_direction_legal(self, direction: Direction, color: PlayerColor):
        if (color == PlayerColor.RED and (
                direction in ILLEGAL_RED_DIRECTIONS)) or \
                (color == PlayerColor.BLUE and (
                        direction in ILLEGAL_BLUE_DIRECTIONS)):
            raise IllegalActionException(
